...
export default function App() {
  ...
  return (
    <div className="bg-bg2 text-t2 font-sans selection:bg-teal-accent selection:text-bg1 min-h-screen relative overflow-x-hidden medical-grid-bg">
      ...
      {/* 1. TOP SYSTEM CONTROL PANEL (COLLAPSIBLE TELEMETRY DOCK) */}
      <div className="border-b border-b2/40 bg-bg1/90 backdrop-blur-md relative z-150">
        <div className="max-w-[1160px] mx-auto px-5 py-2 flex items-center justify-between text-xs font-mono">
          <div className="flex items-center gap-2">
            ...
            <span className="text-tm uppercase tracking-wider">SYSTEM STATUS: LIVE DIALER ROUTING ACTIVE</span>
          </div>
          <button 
            onClick={() => setIsControlPanelOpen(!isControlPanelOpen)}
            className="flex items-center gap-1.5 py-1 px-3 bg-bg3 hover:bg-bg4 border border-b2 hover:border-teal-18 rounded text-teal-accent hover:text-t1 transition-all cursor-pointer text-[11px]"
            id="btn-control-panel-toggle"
          >
            <Settings size={13} className={isControlPanelOpen ? "animate-spin" : ""} />
            {isControlPanelOpen ? "CLOSE ROUTING CONSOLE" : "CONFIGURE LIVE WHATSAPP & CALENDLY"}
          </button>
        </div>
        ...
        <AnimatePresence>
          {isControlPanelOpen && (
            <motion.div 
              ...
              className="overflow-hidden bg-bg3 border-t border-b1"
            >
              <div className="max-w-[1160px] mx-auto px-5 py-6 font-mono text-sm">
                <div className="bg-bg4/60 border border-b2 rounded-xl p-5 md:p-6">
                  <h3 className="text-t1 font-serif text-lg md:text-xl font-medium mb-3 flex items-center gap-2">
                    <Sliders size={18} className="text-teal-accent animate-pulse" />
                    ICU DIALER CONNECTOR
                  </h3>
                  <p className="text-xs text-tm mb-5 leading-relaxed">
                    Set your custom credentials below. Clicking save will dynamically route all booking buttons on this landing page directly to your active Calendly scheduler or WhatsApp chat link instantly.
                  </p>
                  
                  <form onSubmit={handleSaveLinks} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                    <div>
                      <label className="block text-xs text-tm uppercase tracking-wider mb-1.5">Calendly Event link</label>
                      <div className="relative">
                        <Calendar size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-tg" />
                        <input 
                          type="url" 
                          required
                          value={calendlyUrl}
                          ...
                          className="w-full bg-bg5 border border-b2 focus:border-teal-accent/50 text-t1 rounded px-3 py-2 pl-9 text-xs outline-none transition-all"
                        />
                      </div>
                    </div>

                    <div>
                      <label className="block text-xs text-tm uppercase tracking-wider mb-1.5">WhatsApp Phone Number</label>
                      <div className="relative">
                        <Phone size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-tg" />
                        <input 
                          type="text" 
                          required
                    ...
                    <div className="flex gap-2">
                      <div className="flex-1">
                        <label className="block text-xs text-tm uppercase tracking-wider mb-1.5">WhatsApp Message</label>
                        <input 
                          type="text" 
                          value={whatsappText}
                          onChange={(e) => setWhatsappText(e.target.value)}
                          placeholder="Pre-filled message..."
                          className="w-full bg-bg5 border border-b2 focus:border-teal-accent/50 text-t1 rounded px-3 py-2 text-xs outline-none transition-all"
                        />
                      </div>
                      <button 
                        type="submit" 
                        className="bg-teal-accent text-bg1 font-sans font-medium px-4 h-[34px] rounded hover:opacity-90 transition-all cursor-pointer text-xs flex items-center gap-1 self-end"
                        id="btn-save-telemetry-config"
                      >
                        <Check size={14} />
                        SAVE
                      </button>
                  ...
                  <AnimatePresence>
                    {isSavedNoticeVisible && (
                      <motion.div 
                        ...
                        exit={{ opacity: 0, y: -10 }}
                        className="mt-4 p-3 bg-teal-10 border border-teal-18 text-teal-accent rounded-lg flex items-center gap-2 text-xs"
                      >
                        <CheckCircle2 size={14} />
                        <span>TELEMETRY CONNECTIONS APPLIED SUCCESSFULLY! All CTA buttons are now routing live. Try clicking any button to test.</span>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
      ...
      {/* 2. FIXED NAVIGATION */}
      <nav id="nav" className={`fixed top-[41px] left-0 right-0 z-100 h-[66px] flex items-center transition-all ${isNavScrolled ? "bg-bg1/95 border-b border-b3/25 shadow-[0_4px_30px_rgba(0,0,0,0.5)] backdrop-blur-md" : "bg-bg1/90 border-b border-b1"}`}>
